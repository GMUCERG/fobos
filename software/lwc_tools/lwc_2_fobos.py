...
def get_pdi_str(pdi, rand, width, shares):
...
def gen_test_vectors(tv_type, num_vectors, width, shares, pdi_file, sdi_file, do_file, fobos_din_file='dinFile.txt', fobos_fvr_file='fvrchoicefile.txt'):
    # tv_type = 'tvla' or 'random'
    pdi_file = open(pdi_file, 'r')
    sdi_file = open(sdi_file, 'r')
    do_file = open(do_file, 'r')
    ...
    shared_do_len = do_len * shares # do.txt is not shared but output is

    din_file = open(fobos_din_file, 'w')
    ...
    if tv_type=='tvla':
        fvr_file = open(fobos_fvr_file, 'w')
        for i in range(num_vectors):
            coin = random.randint(0,1)
            if coin==1:
                # random
                pdi_str = get_pdi_str(pdi, True, width, shares)
                fvr_file.write('1')
            else:
                # fixed
                pdi_str = get_pdi_str(pdi, False, width, shares)
                fvr_file.write('0')
